...
public class HelpScreen2 extends HelpScreen {
  ...
  enum MotionStages {
    MotionPause, AntiVirusLeft, AntiVirusDownLeft, AntiVirusRight, AntiVirusUpRight
  }
  ...
  private void initialize() {
    ...
    antiVirusStartY = textY + Assets.helpScreen2Text.getHeight()
        + (int) (utils.CANVAS_HEIGHT * 0.2);
  ...
  private void updateViruses(float deltaTime) {
    ...
    if (motionStage == MotionStages.AntiVirusLeft && motionStageLengthRemaining <= 0.1
        && !topVirusCreated) {
    ...
    if (motionStage == MotionStages.AntiVirusLeft
        && motionStageLengthRemaining == motionStageLength) {
      viruses.push(new Virus(utils.CANVAS_WIDTH, rightVirusY, (-1 * virusSpeed), 0, utils));
    }
