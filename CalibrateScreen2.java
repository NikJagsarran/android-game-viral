...
public class CalibrateScreen2 extends Screen {
  ...
  private void initialize() {
    ...
    continueButton = new Button(
        (utils.CANVAS_WIDTH - Assets.calibrateScreen2Continue.getWidth()) / 2, doneY
            + (int) (utils.CANVAS_HEIGHT * 0.2), Assets.calibrateScreen2Continue,
        Assets.calibrateScreen2ContinueClicked);

    angleStringRight = utils.CANVAS_WIDTH / 2;
    angleStringY = (int) (utils.CANVAS_HEIGHT * 0.25);

    faceTextY = angleStringY + Assets.newScoreDigits.getHeight()
        + (int) (utils.CANVAS_HEIGHT * 0.05);
  ...
  private void updateCalibrating(float deltaTime) {
    ...
    if (timePassed < timeToHold) {
      ...
      if ((lastAccelY * currAccelY < 0)
          && (Math.abs(lastArctan) > ((Math.PI / 2) - tolerance) || Math.abs(currArctan) > ((Math.PI / 2) - tolerance))) {
  ...
  private void presentCalibrating(Graphics g) {
    ...
    if (angleUpdated) {
      ...
      angleStringRight = (utils.CANVAS_WIDTH + (angleString.length() - 1)
          * Constants.SCORE_DIGIT_WIDTH) / 2;
